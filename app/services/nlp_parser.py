"""
Natural Language Processing service for parsing parent queries
"""
...
class ParentQueryParser:
    """Parse natural language queries from parents into structured search parameters"""
    ...
    async def parse_query(self, query: str) -> Dict:
        ...
        Example: "Basketball classes for 8-10 near me this Saturday"
        Returns: {
            "age_min": 8,
            "age_max": 10,
            "category": "sports",
            ...
            "suggestions": ["Do you want indoor or outdoor basketball?"]
        }
        """
        ...
        # Use AI for advanced parsing and suggestions
        ai_result = await self._ai_parse_query(query)
        if ai_result:
            result["parsed_params"].update(ai_result.get("parsed_params", {}))
    ...
    async def _ai_parse_query(self, query: str) -> Optional[Dict]:
        """Use AI to parse complex queries and provide suggestions"""
        try:
            prompt = f"""
            Parse this parent's query for kids' activities and extract structured information:
            
            Query: "{query}"
            
            Extract and return a JSON object with:
            1. parsed_params: structured search parameters
            2. suggestions: helpful follow-up questions if information is missing
            3. confidence: how confident you are in the parsing (0-1)
            
            Focus on:
            - Age ranges (age_min, age_max)
            - Activity categories (sports, arts, education, etc.)
            - Location preferences (indoor/outdoor, specific venues)
            - Time preferences (weekends, weekdays, specific days)
            - Cost preferences (free, paid)
            
            If information is missing, suggest helpful follow-up questions.
