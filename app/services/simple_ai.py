"""
Simplified AI service with LangChain and Anthropic integration
"""
...
class SimpleEventPersonalizationAgent:
    """Simplified AI agent for event personalization using LangChain"""
    ...
    async def personalize_events(
        ...
        """Personalize event recommendations using AI"""
        ...
        try:
            ...
            summary = self._generate_simple_summary(events, user_profile, zip_code)
        
        # Score and rank events based on user preferences
        scored_events = self._score_events(events, user_profile)
    ...
    def _generate_simple_summary(self, events: List[Event], user_profile: Optional[Dict[str, Any]], zip_code: str) -> str:
        """Generate a simple summary without AI (fallback)"""
        
        if user_profile and user_profile.get("children_ages"):
            ages = user_profile["children_ages"]
            age_text = f"ages {min(ages)}-{max(ages)}" if len(ages) > 1 else f"age {ages[0]}"
            return f"Here are {len(events)} great activities for kids {age_text} near {zip_code}!"
        
        return f"Here are {len(events)} great family activities near {zip_code}!"
    
    def _score_events(self, events: List[Event], user_profile: Optional[Dict[str, Any]]) -> List[Event]:
        """Score and rank events based on user preferences"""
...
class SimpleEventSummarizationAgent:
    """Simplified AI agent for generating event summaries using LangChain"""
    ...
    async def generate_event_summary(self, event: Event) -> str:
        ...
        try:
            ...
            logger.error(f"AI summary generation error: {e}")
            # Fallback to simple summary
            location_type = "Indoor" if event.is_indoor else "Outdoor"
            return f"A fun {event.category or 'family'} activity at {event.location_name or 'this location'}. Perfect for {age_range}, {location_type.lower()}, {cost_info.lower()}."
    
    async def generate_batch_summaries(self, events: List[Event]) -> Dict[int, str]:
        """Generate AI summaries for multiple events"""
        ...
        for i in range(0, len(events), batch_size):
            ...
            for j, summary in enumerate(batch_summaries):
                if isinstance(summary, str):
                    ...
                    cost_info = "Free" if event.is_free else "Paid event"
                    location_type = "Indoor" if event.is_indoor else "Outdoor"
                    summaries[event.id] = f"A fun {event.category or 'family'} activity at {event.location_name or 'this location'}. Perfect for {age_range}, {location_type.lower()}, {cost_info.lower()}."
        
        return summaries


# Global instances
simple_personalization_agent = SimpleEventPersonalizationAgent()
simple_summarization_agent = SimpleEventSummarizationAgent()
